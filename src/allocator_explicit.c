...
static header_t *last_found_slot = NULL;
...
void allocator_init(size_t init_size, uint8_t pol) {
    mem_start = (char *)malloc(init_size);
    if(mem_start == NULL) {
...
void list_remove(header_t* block) {
    if(last_found_slot == block) last_found_slot = block->next;
    if(block->prev) block->prev->next = block->next;
    else free_list_head = block->next;
    if(block->next) block->next->prev = block->prev;
}

void list_push_front(header_t* block) {
...
header_t* findBestFit(size_t size) {
    header_t *temp = free_list_head; header_t *best_slot = NULL;
    size_t best = INT_MAX;
    while(temp) {
        if(temp->size >= size) {
            size_t diff = temp->size - size;
            if(best > diff) {
                best = diff;
                best_slot = temp;
            }
...
header_t* findWorstFit(size_t size) {
    header_t *temp = free_list_head; header_t *worst_slot = NULL;
    size_t worst = 0;
    while(temp) {
        if(temp->size >= size) {
            size_t diff = temp->size - size;
...
header_t* findNextFit(size_t size) {
    next_fit_calls++;
    header_t *start = last_found_slot ? last_found_slot : free_list_head;
    header_t *temp = start;
    while(temp) {
        next_fit_nodes_visited++;
        if(temp->size >= size) {
            last_found_slot = temp->next;
            next_fit_pass1_hits++;
            return temp;
        }
        temp = temp->next;
...
header_t* joinL(header_t* left, header_t* curr) {
    list_remove(left);
    ...
    if((char *)left != mem_start) {
        ...
        if(prev_h->free == 1) return joinL(prev_h, left);
    }
    return left;
}
...
size_t allocator_largest_free(void) {
    ...
    header_t *temp = free_list_head;
    while(temp) {
        if(temp->size > largest) largest = temp->size;
        temp = temp->next;
    }
    return largest;
}
...
void custom_free(void *ptr) {
    ...
    header_t *final_block = curr_header;
    if((char *)curr_header != mem_start) {
        ...
        if(left_h->free == 1) final_block = joinL(left_h, curr_header);
    }

    list_push_front(final_block);
}